                    ...
                    <div className="flex-grow p-4 overflow-y-auto">
                        {messages.map((msg, index) => (
                            <div key={index} className={`flex items-end mb-3 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
                                <div className={`px-3 py-2 rounded-lg ${msg.sender === 'bot' ? 'bg-green-100 text-green-900' : 'bg-gray-200 text-gray-800'}`}>
                                    {msg.text}
                                </div>
                            </div>
                        ))}
                        {loading && (
                            <div className="flex items-end mb-3">
                                <div className="px-3 py-2 rounded-lg bg-green-100 text-green-900">
                                    Typing...
                                </div>
                            </div>
                        )}
