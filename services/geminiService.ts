...
const memoFicheItemSchema = {
    ...
    properties: {
        ...
        summaryVideoUrl: { type: Type.STRING, description: "Optional YouTube video URL for the summary section." },
        memoContent: {
            type: Type.ARRAY,
    ...
    if (theme.Nom.trim().toLowerCase() === "ordonnances") {
        ...
        prompt = `
            ...
            **Instructions de Génération Spécifiques pour Ordonnances:**
            ...
            - **Vidéo YouTube**: ${options.videoUrl ? `Utilise CETTE URL EXACTE pour 'youtubeUrl': ${options.videoUrl}` : "Si pertinent, fournis un lien YouTube pertinent pour le résumé dans 'youtubeUrl'."}
            - **Vidéo de Résumé**: Si pertinent, fournis un lien YouTube pour la section de résumé dans 'summaryVideoUrl'.
            - **Ressources Externes**: 
...
content
 sera le texte de ce paragraphe. La première section doit toujours être un 'Aperçu' qui résume brièvement le sujet.
